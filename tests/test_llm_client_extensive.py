"""Extensive tests for LLM Client module - targeting 80% coverage"""
...
class TestLLMAgentClientExtensive(unittest.TestCase):
    """Extensive tests for LLMAgentClient"""
    ...
    @patch('socket.socket')
    def test_send_and_receive_empty_response(self, mock_socket_class):
        """Test _send_and_receive with empty response"""
        mock_socket = MagicMock()
        mock_socket_class.return_value = mock_socket
        self.client.socket = mock_socket
    ...
    @patch('socket.socket')
    def test_send_and_receive_unexpected_message_type(self, mock_socket_class):
        """Test _send_and_receive with unexpected message type"""
        mock_socket = MagicMock()
        mock_socket_class.return_value = mock_socket
        self.client.socket = mock_socket
        
        # Create message with unexpected type
        payload = BinaryProtocol.encode_json({"data": "test"})
        response = BinaryProtocol.pack_message(0xFF, payload)  # Unknown type
