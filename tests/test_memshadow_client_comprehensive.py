"""Comprehensive tests for MEMSHADOW Client module"""
...
class TestMRACClientComprehensive(unittest.TestCase):
    """Comprehensive tests for MRACClient"""
    ...
    def test_sequence_number_large(self):
        """Test sequence number with large value"""
        self.client.sequence_num = 2**31 - 1
        seq = self.client._next_sequence()
        self.assertEqual(seq, 2**31)  # Should increment normally
    ...
    def test_connect_failure(self, mock_socket_class):
        ...
        # Just verify connect was attempted
    ...
    @patch('modules.memshadow_client.MRACProtocol')
    @patch('modules.memshadow_client.MemshadowHeader')
    def test_send_heartbeat_no_socket(self, mock_header, mock_protocol):
